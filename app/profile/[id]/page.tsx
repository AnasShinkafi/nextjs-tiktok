...
const ProfilePage = ({ params }: ProfilePageTypes) => {
    ...
    return (
        <>
            <MainLayout>
                <div className=" pt-[90px] ml-[90px] 2xl:pl-[185px] lg:pl-[160px] lg:pr-8 w-[calc(100% -98px)] pr-3 max-w-[1800px] 2xl:mx-auto">
                    <div className=" flex w-[calc(100vw-230px)]">
                        <ClientOnly>
                            {currentProfile ? (
                                <img className="w-[120px] min-w-[120px] rounded-full" src={useCreateBucketUrl(currentProfile?.image)} alt={currentProfile?.name} />
                            ) : (
                        ...
                        <div className=" ml-5 w-full">
                            ...
                            {contextUser?.user?.id == params?.id ? (
                                ...
                                <button
                                    className=" flex items-center rounded-md py-1.5 px-8 mt-3 text-[15px] text-white font-semibold bg-[#F02c56]">Follow</button>
                            )}
                        </div>
                    </div>
                    ...
                    <ul className=" w-full flex items-center pt-4 border-b">
                        <li className=" w-60 text-center pt-4 text-[17px] font-semibold border-b-2 border-b-black">Videos</li>
                        <li className=" w-60 text-gray-500 text-center py-2 text-[17px] font-semibold">Likes</li>
                    </ul>
                    <ClientOnly>
                        <div className=" mt-4 grid 2xl:grid-cols-6 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-3">
                            {postByUser?.map((post, key) => (
                                <PostUser key={key} post={post} />
                            ))}
                        </div>
                    </ClientOnly>
                </div>
