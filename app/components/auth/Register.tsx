            ...
            <div className="">
                ...
                <div className=" px-6 mt-6 pb-2">
                    <button 
                        disabled={loading}
                        onClick={() => register()}
                        className={`flex items-center py-4 justify-center w-full text-[17px] font-semibold text-white ${(!name || !email || !password || !confirmPassword) ? 'bg-gray-200' : "bg-[#F02C56]"}`}>
                            {loading ? <BiLoaderCircle size="25" className="animate-spin" color="#ffffff"/> : "Register"}
                        </button>
                </div>
            </div>
        </>
