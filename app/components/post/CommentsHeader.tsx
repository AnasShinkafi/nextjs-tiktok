...
const CommentsHeader = ({ post, params }: CommentsHeaderCompTypes) => {
    ...
    useEffect(() => { hasUserLikePost() },[likesByPost]);
    ...
    const deletePost = async () => {
        let res = confirm("Are you sure you wnt to delete the post?");
        if(!res) return;
    ...
    return (
        <>
            <div className=" flex items-center justify-between px-8">
                <div className=" flex items-center">
                    <Link href={`/profile/${post?.user_id}`}>
                        {post?.profile.image ? (
                            <img className=" rounded-full lg:mx-0 mx-auto" width="40" src={useCreateBucketUrl(post?.profile.image)} alt="" />
                        ) : (
            ...
            <p className=" px8 mt-4 text-sm">{post?.text}</p>
            ...
            <div className=" flex items-center px-8 mt-8">
                <ClientOnly>
                    <div className=" pb-4 text-center flex items-center">
                        <button 
                            ...
                            className=" rounded-l-full bg-gray-200 p-2 cursor-pointer">
                                {hasClickedLike ? (
                                    <AiFillHeart color={likesByPost.length > 0 && userLiked ? "#ff2656" : ''} size="25"/>
                                ) : (
                                    <BiLoaderCircle className="animate-spin" size="25" />
                                )}
                            </button>
                ...
                <div className=" pb-4 text-center flex items-center">
                    ...
                    <span className=" text-xs pl-2 to-gray-800 font-semibold">
